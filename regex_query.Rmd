...
The log(view_count) model fits a lot better, with saner diagnostics and this makes sense given the distribution of views which has a long upper tail.

Under this model the ranking of concerns is different: whitespace, escaping, validation, substring, match, lookarounds, rewrite, negation, greedy, replace, parsing.

Intuitively this ranking feels better to me. Whitespace and escaping are definitely common concerns.
...
It seems 'string' is a catch all category for splitting, extraction - numbers, substitution and removal - particularly dealing with whitespace. 
